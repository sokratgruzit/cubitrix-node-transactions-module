...
const TOKEN_HOLDER_TREASURY_ADDRESS = process.env.TOKEN_HOLDER_TREASURY_ADDRESS;
...
async function check_transactions_for_pending(io) {
  ...
  const updatePromises = get_txs.map(async (tx) => {
    ...
    // console.log(data.exchange, exchangeId, "status");
...
async function make_withdrawal(req, res) {
  ...
  try {
    ...
    if (accountType === "ATAR") {
      ...
      if (
        ...
        if (treasury) {
          ...
          incoming.ATR = balance;

          await treasuries.findOneAndUpdate(
            {},
            { incoming: incoming },
            { new: true }
          );
    ...
    if (pendingWithdrawalAmount + amount > currentIncomingAmount) {
      //Here we need to add check of real treasury balance for currency
      return res
        .status(400)
        .json(
          main_helper.error_message(
...
async function direct_deposit(req, res) {
  try {
    let { hash, account_currency } = req.body;
    ...
    if (
      ...
      const tokenAmount = numberOfTokens.dividedBy(precision);
