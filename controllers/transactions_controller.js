...
async function create_exchange_transaction(req, res) {
  try {
    ...
    let { tokenAddress, amount, decimals, isNative, from } = req.body;
  ...
  try {
    ...
    if (data?.dataa?.exchange?.status == "success") {
      ...
      if (transactions.status == "pending") {
        await change_balance_and_tx_status(
          exchangeIdAsObjectId,
          get_tx,
          ratesObj
        );
      }
...
async function change_balance_and_tx_status(
  exchangeIdAsObjectId,
  get_tx,
  ratesObj
) {
